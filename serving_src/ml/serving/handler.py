...
class AsteriaHandler(BaseHandler):
    ...
    def handle(self, data, context):
        batch = data[0].get("body")
        inputs = json.loads(batch)

        emb = torch.tensor(inputs["embedding"]).float()
        feats = torch.tensor(inputs["features"]).float()

        if emb.shape[0] < 384:
            emb = torch.nn.functional.pad(emb, (0, 384 - emb.shape[0]))
        if feats.shape[0] < 10:
            feats = torch.nn.functional.pad(feats, (0, 10 - feats.shape[0]))

        x = torch.cat([emb, feats], dim=0).unsqueeze(0).unsqueeze(0)
